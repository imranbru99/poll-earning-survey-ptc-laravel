...
class SurveyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $surveys = Survey::where('publisher_user_id',$user->id)->latest()->with(array(
        ...
        return view('publisher.survey.index', compact('surveys', 'page_title','empty_message'));
    }
    ...
    public function ExportSurvey(Request $request)
    {
        ...
        $ab= DB::table('completed_surveys')
            ...
            ->select('users.username','surveys.name','surveys.amount','completed_surveys.created_at')
            ->groupBy('completed_surveys.survey_id')
            ->get();
    ...
    public function create(Request $request)
    {
        ...
        if($user->balance >= 1000){
            ...
            return view('publisher.survey.create', compact(['page_title', 'categories']));  
        }else{
            $notify[] = ['success', 'To Create New Survey, Your Minimum balance Need 1500 Taka'];
...
//        dd('okok');
    ...
    {
        $user = PublisherUser::where('username',Session::get('PublisherUserLogin'))->first();
        $setupSurvey = Survey::create($request->validated() + ['surveySlug' => Str::slug($request['name']),'publisher_user_id' => $user->id,'active' => 0]);
        $request->session()->flash('survey-created', 'Survey created successfully');
        return redirect()->route('publisher_user.create');
    }
    ...
    public function indexResult($id)
    {
        ...
        return view('publisher.survey.surveyResult.index', compact('page_title', 'question', 'answers'));
    }
    ...
    public function indexInputResult($id)
    {
        ...
        $answers =  ComplatedSurvey::where('survey_question_id', $id)->get();
        $question = SurveyQuestion::find($id)->question;
        return view('publisher.survey.surveyResult.indexInput', compact('page_title', 'question', 'answers'));
    }
...
//        dd($result);
        $question = SurveyQuestion::find($id)->question;
        return view('publisher.survey.surveyResult.indexMulti', compact('page_title', 'question', 'result'));
    }
    ...
    public function indexYesno($id)
    {
        ...
        $question = SurveyQuestion::find($id)->question;
        return view('publisher.survey.surveyResult.indexYesno', compact('page_title', 'question', 'yes', 'no'));
    }
    public function iqQuestion($id)
    {
        ...
        return view('publisher.survey.surveyResult.indexIq', compact('page_title', 'question', 'answers'));
    }
...
//        dd($answers);
        return view('publisher.survey.surveyResult.indexdropdown', compact('page_title', 'question', 'answers'));
    }
