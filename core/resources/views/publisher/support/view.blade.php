...
<section>
<script type="application/javascript" src="https://syndication.exdynsrv.com/splash.php?idzone=4475908"></script>
    <div class="container">
                                ...
                                <div id="collapseThree" class="collapse show"
                                    ...
                                    <div class="card-body">
                                        @if($my_ticket->status != 4)
                                            <form method="post"
                                                  action="{{ route('publisher_user.ticket.reply', $my_ticket->id) }}"
                                                        ...
                                                        @if($message->attachments()->count() > 0)
                                                            <div class="mt-2">
                                                                @foreach($message->attachments as $k=> $image)
                                                                    <a href="{{route('publisher_user.ticket.download',encrypt($image->id))}}" class="mr-3"><i class="fa fa-file"></i>  @lang('Attachment') {{++$k}} </a>
                                                                @endforeach
                                                            </div>
                                                        @endif
...
<div class="modal fade" id="DelModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" action="{{ route('publisher_user.ticket.reply', $my_ticket->id) }}">
                @csrf
                @method('PUT')
