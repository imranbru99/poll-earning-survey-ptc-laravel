...
class PtcController extends Model
{
	public function index(){
        ...
        $ads = Ptc::where('remain','>', 0)
                    ...
                    ->Where('showed', '>', -10)
                    ->where('status',1)
                    ->inRandomOrder()->orderBy('remain','ASC')->limit(10)->get();

    	$viewed = PtcView::where('user_id',auth()->user()->id)->where('vdt',Date('Y-m-d'))->pluck('ptc_id')->toArray();

    	$empty_message = "We are developing ads panel";
    ...
    public function confirm($hash)
    {
        ...
        $notify[] = ['success','Successfully viewed this ads, Tk has been added in your balance'];
        return redirect()->route('user.ptc.index')->withNotify($notify);
    }

    public function clicks()
    {
        $page_title = "Ads Clicks History";
