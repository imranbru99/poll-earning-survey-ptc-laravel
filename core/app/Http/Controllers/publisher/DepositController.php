...
class DepositController extends Controller
{
    public function deposit()
    {
        ...
        return view('publisher.deposit.deposit', compact('gatewayCurrency', 'page_title'));
    }
    ...
    public function depositInsert(Request $request)
    {
        ...
        $user = $pUser = PublisherUser::where('username',Session::get('PublisherUserLogin'))->first();
        ...
        $depo['publisher_user_id'] = $user->id;
        ...
        return redirect()->route('publisher_user.deposit.preview');
    }
    ...
    public function depositPreview()
    {
        ...
        if (is_null($data)) {
            ...
            return redirect()->route('publisher_user.deposit')->withNotify($notify);
        }
        if ($data->status != 0) {
            $notify[] = ['error', 'Invalid Deposit Request'];
            return redirect()->route('publisher_user.deposit')->withNotify($notify);
        ...
        return view('publisher.deposit.preview', compact('data', 'page_title'));
    }
    ...
    public function depositConfirm()
    {
        ...
        return view('publisher' . $data->view, compact('data', 'page_title', 'deposit'));
    }
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            $user = PublisherUser::find($data->publisher_user_id);
            ...
            $transaction->publisher_user_id = $data->publisher_user_id;
    ...
    public function manualDepositConfirm()
    {
        ...
        if (!$data) {
            return redirect()->route('publisher_user.deposit');
        }
        if ($data->status != 0) {
            return redirect()->route('publisher_user.deposit');
        }
        if ($data->method_code > 999) {
            ...
            return view('publisher.deposit.manual_confirm', compact('data', 'page_title', 'method'));
        }
        abort(404);
    }
    ...
    public function manualDepositUpdate(Request $request)
    {
        ...
        $track = session()->get('Track');
        $data = Deposit::with('gateway','publish_user')->where('status', 0)->where('trx', $track)->first();
        if (!$data) {
            return redirect()->route('publisher_user.deposit');
        }
        if ($data->status != 0) {
        ...
        $data->update();
        // return $data->publish_user;
        $gnl = GeneralSetting::first();
        notify($data->publish_user, 'DEPOSIT_REQUEST', [
            'method_name' => $data->gateway_currency()->name,
            'method_currency' => $data->method_currency,
            'method_amount' => getAmount($data->final_amo),
            'amount' => getAmount($data->amount),
        ...
        return redirect()->route('publisher_user.deposit.history')->withNotify($notify);
    }
    ...
    public function depositHistory()
    {
        ...
        $logs = $user->deposits()->with(['gateway'])->latest()->paginate(getPaginate());
        return view('publisher.deposit.deposit_history', compact('page_title', 'empty_message', 'logs'));
    }
}
